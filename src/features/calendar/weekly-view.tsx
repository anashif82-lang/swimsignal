...
const TYPE_STYLE: Record<string, string> = {
  water:   "border-signal-400   bg-signal-400/15   text-signal-300",
  dryland: "border-warning-400  bg-warning-400/15  text-warning-300",
  gym:     "border-navy-500     bg-navy-800         text-navy-300",
  other:   "border-navy-500     bg-navy-800         text-navy-300",
};
  ...
  return (
    <>
      ...
      {open && (
        ...
        >
          <div
            className="bg-navy-900 border border-surface-border rounded-xl p-4 w-72 space-y-3 shadow-xl"
          ...
          >
            ...
            {session.notes && <p className="text-xs text-navy-300">{session.notes}</p>}
    ...
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="flex items-center justify-between mb-3 shrink-0">
        <div className="flex items-center gap-1">
          <button onClick={goBack} className="p-1.5 rounded-lg text-navy-400 hover:text-white hover:bg-navy-800 transition-colors">
            <ChevronRight className="h-4 w-4" />
          </button>
          <span className="text-sm font-medium text-white px-1">{rangeLabel}</span>
          <button onClick={goNext} className="p-1.5 rounded-lg text-navy-400 hover:text-white hover:bg-navy-800 transition-colors">
            <ChevronLeft className="h-4 w-4" />
          </button>
        </div>
        <button onClick={goNow} className="text-xs text-signal-400 hover:text-signal-300 transition-colors px-2 py-1 rounded-lg hover:bg-signal-400/10">
          היום
        </button>
      </div>
      ...
      <div className="flex-1 overflow-auto rounded-xl border border-surface-border bg-navy-950">
        <div className="flex" style={{ minWidth: "520px" }}>
          ...
          {/* Time axis */}
          <div className="w-10 shrink-0 bg-navy-950 sticky start-0 z-10">
            <div className="h-10 border-b border-surface-border" />
            {HOURS.map((h) => (
              <div
                key={h}
                style={{ height: HOUR_PX }}
                className="border-b border-surface-border/30 flex items-start justify-end pe-1.5 pt-1"
              >
                <span className="text-[9px] text-navy-600 font-mono">{String(h).padStart(2, "0")}</span>
              </div>
            ))}
          </div>
          ...
          {weekDates.map((date, i) => {
            ...
            return (
              <div
                key={iso}
                className={cn(
                  "flex-1 border-s border-surface-border min-w-0",
                  isToday && "bg-signal-400/[0.03]"
                )}
              >
                {/* Day header */}
                <div className={cn(
                  "h-10 flex flex-col items-center justify-center border-b border-surface-border sticky top-0 z-10",
                  isToday ? "bg-signal-400/10" : "bg-navy-950"
                )}>
                  <span className="text-[9px] text-navy-500 uppercase">{DAY_NAMES[date.getDay()]}</span>
                  <span className={cn(
                ...
                >
                  {HOURS.map((h) => (
                    <div
                      ...
                      className="absolute inset-x-0 border-b border-surface-border/20 hover:bg-navy-900/40 transition-colors cursor-pointer"
